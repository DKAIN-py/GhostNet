  ...
  return (
    <div className="flex flex-col h-full" style={{ fontFamily: T.font.mono }}>
      ...
      <div className="flex-1 overflow-y-auto" style={{ maxHeight: '240px' }}>
        {feed.map((sig, i) => {
          const sev        = getSeverityStyle(sig.anomalyLevel);
          ...
          return (
            <div key={`${sig.agentId}-${sig.timestamp}-${i}`}
              ...
              <span className="text-[9px] tracking-widest font-bold w-6 shrink-0"
                style={{ color: isCritical ? T.text.primary : T.text.muted }}>
                {ABBR[sig.agentId]}
              </span>
              <span className="w-[4px] h-[4px] rounded-full shrink-0"
                style={{ background: sev.border }} />
              <span className="text-[10px] flex-1 truncate" style={{ color: T.text.secondary }}>
                {sig.signal}
                <span className="ml-1" style={{ color: T.text.micro }}>· {sig.anomalyLevel?.toUpperCase()}</span>
              </span>
              <span className="text-[9px] shrink-0" style={{ color: T.text.micro }}>
