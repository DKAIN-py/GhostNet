...
export default function CascadeBar() {
  ...
  if (!cascade) {
    return (
      <div className="flex items-center justify-between px-4 py-3"
        ...
        <button onClick={fireFakeCascade}
  ...
  // ── ALARM STATE — structural inversion ──────────────────
  return (
    <div className="flex flex-col gap-3 px-4 py-4"
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={clearCascade}
          className="text-[10px] tracking-widest uppercase px-3 py-1 font-bold"
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <CascadeStat label="RECOMMENDATION"   value={cascade.recommendation}    />
      </div>
...
function CascadeStat({ label, value }) {
  return (
    <div className="flex flex-col gap-1">
      <span className="text-[8px] tracking-widest uppercase" style={{ color: 'rgba(252,250,245,0.4)' }}>{label}</span>
      <span className="text-[11px] font-bold leading-tight" style={{ color: T.cascade.text }}>{value}</span>
    </div>
  );
}